...
#     @patch('main.bots.SCALE_T.csv_utils.csv_service.get_logger')
#     @patch('main.bots.SCALE_T.csv_utils.csv_core.get_ticker_filepath')
...
#         # Create instance
#         csv_service = CSVService(self.ticker, self.trading_type)
        
#         # Verify logger initialization and calls
#         mock_get_logger.assert_called_once_with("csv_service")
#         mock_logger.info.assert_any_call(f"Initializing CSVService for {self.ticker} ({self.trading_type}) with custom_id: None")
#         # mock_logger.debug.assert_any_call(f"Names and path complete. Setting up metadata and columns.")
#         mock_logger.info.assert_any_call("CSVService initialized successfully.")
        
#         # Verify filepath setup and file operations
#         mock_get_filepath.assert_called_once_with(self.ticker, self.trading_type, None)
#         self.assertEqual(csv_service.csv_filepath, self.mock_filepath)
...
#         self.assertEqual(csv_service.ticker, "AAPL")
#         self.assertEqual(csv_service.trading_type, self.trading_type)
#         self.assertIsNone(csv_service.custom_id)
        
#         # Verify metadata and required columns
#         self.assertEqual(csv_service.metadata, self.mock_metadata)
...
#         ]
#         self.assertEqual(sorted(csv_service.required_columns), sorted(expected_required_columns))
        
#         # Verify loaded CSV data
#         self.assertEqual(csv_service.csv_data, self.sample_csv_data)
...
#         csv_service = CSVService(self.ticker, LIVE_TRADING)
        
#         self.assertEqual(csv_service.trading_type, LIVE_TRADING)
...
#         csv_service = CSVService(self.ticker, self.trading_type, custom_id)
        
#         self.assertEqual(csv_service.custom_id, custom_id)
...
#             CSVService(self.ticker, "invalid_type")
...
#         self.assertEqual(csv_service.metadata, {})
#         self.assertEqual(csv_service.required_columns, [])

#     @patch('main.bots.SCALE_T.csv_utils.csv_core.CSVCore._load_csv_data')
#     @patch('main.bots.SCALE_T.csv_utils.csv_core.get_ticker_filepath')
#     @patch('builtins.open', new_callable=mock_open)
#     @patch('json.load')
#     def test_init_invalid_metadata_format(self, mock_json_load, mock_file, mock_get_filepath, mock_load_csv_data):
#         """Test handling of invalid metadata format."""
#         # Setup invalid metadata format
#         mock_json_load.return_value = {"invalid": "format"}
#         mock_load_csv_data.return_value = self.sample_csv_data
...
#         required_cols = csv_service._get_required_columns()
...
#         csv_service = CSVService(self.ticker, self.trading_type)
#         with patch.object(csv_service, 'validate_csv_data', return_value=True):
#             data = csv_service._load_csv_data(self.mock_filepath)
...
#         csv_service = CSVService(self.ticker, self.trading_type)
#         with patch.object(csv_service, 'validate_csv_data', return_value=False):
#             with self.assertRaises(ValueError) as context:
#                 csv_service._load_csv_data(self.mock_filepath)
...
#         data = csv_service._load_csv_data(non_existent_file)
...
#     # @patch('main.bots.SCALE_T.csv_utils.csv_service.open', new_callable=mock_open)
#     # @patch('main.bots.SCALE_T.csv_utils.csv_service.csv.DictReader')
...
#     #     from main.bots.SCALE_T.csv_utils.csv_service import CSVService as OriginalCSVService
#     #     original_load_csv_data = OriginalCSVService._load_csv_data
        
#     #     # Monkey patch the class
#     #     original_init = CSVService.__init__
#     #     original_validate = CSVService.validate_csv_data
...
#     #     CSVService.__init__ = mock_init
#     #     CSVService.validate_csv_data = mock_validate
#     #     CSVService.mock_metadata = self.mock_metadata
...
#     #         with patch.object(csv_service, '_load_csv_data', side_effect=original_load_csv_data):
#     #             # We need to call the method directly for the file open mock to be triggered
#     #             result = original_load_csv_data(csv_service, "/mock/path/AAPL.csv")
...
#     #         CSVService.__init__ = original_init
#     #         CSVService.validate_csv_data = original_validate

#     # @patch('main.bots.SCALE_T.csv_utils.csv_service.open')
...
#     #     CSVService.__init__ = mock_init
#     #     CSVService.mock_metadata = self.mock_metadata
#     #     CSVService.sample_csv_data = self.sample_csv_data
...
#     #         # Test the method
#     #         result = csv_service.get_csv_data()
...
#     # @patch('main.bots.SCALE_T.csv_utils.csv_service.open', new_callable=mock_open)
#     # @patch('main.bots.SCALE_T.csv_utils.csv_service.csv.DictWriter')
...
#     #         # Test the method
#     #         csv_service._save_csv_data("/mock/path/AAPL.csv", self.sample_csv_data)
...
#     #         with patch.object(csv_service, '_save_csv_data') as mock_save:
#     #             csv_service.save()
...
#     #         csv_service.update_row(0, updates)
            
#     #         # Assertions
#     #         self.assertEqual(csv_service.csv_data[0]["held_shares"], "5")
#     #         self.assertEqual(csv_service.csv_data[0]["unrealized_profit"], "10.0")
#     #     finally:
#     #         # Restore original method
#     #         CSVService.__init__ = original_init

#     @patch('main.bots.SCALE_T.csv_utils.csv_service.get_logger') # Mock logger to avoid side effects
#     @patch('main.bots.SCALE_T.csv_utils.csv_core.CSVCore._load_csv_data')
#     def test_get_row_by_index(self, mock_load_csv_data, mock_get_logger):
#         """Test get_row_by_index method."""
#         mock_load_csv_data.return_value = self.sample_csv_data
#         csv_service = CSVService(self.ticker, self.trading_type)

#         # with patch.object(CSVService, '_load_metadata', return_value={}), \
#         #      patch.object(CSVService, '_load_csv_data', return_value=[]):
#         #     csv_service = CSVService(self.ticker, self.trading_type)

#         csv_service.csv_data = self.sample_csv_data # Set data attribute directly
        
#         # Test the method for existing index (passed as int)
#         result_int = csv_service.get_row_by_index(1)
...
#         result_str = csv_service.get_row_by_index("1")
...
#         result_none = csv_service.get_row_by_index(999)
...
#         with patch.object(CSVService, '_load_metadata', return_value=self.mock_metadata), \
#              patch.object(CSVService, '_load_csv_data', return_value=[]):
#             csv_service = CSVService(self.ticker, self.trading_type)
...
#         ]
#         csv_service.csv_data = valid_data
#         # Need column names for the check within validate_csv_data
#         with patch.object(csv_service, '_get_column_names', return_value=list(valid_data[0].keys())):
#             self.assertTrue(csv_service.validate_csv_data(), "Validation failed for valid data")

#         # Test with missing required column
#         invalid_data_missing_col = [
#              {"index": "0", "buy_price": "100.0", "target_shares": "10"} # Missing many required cols
#         ]
#         csv_service.csv_data = invalid_data_missing_col
#         # Need column names for the check
#         with patch.object(csv_service, '_get_column_names', return_value=list(invalid_data_missing_col[0].keys())):
#             self.assertFalse(csv_service.validate_csv_data(), "Validation passed with missing required column")
...
#             }
#         ]
#         csv_service.csv_data = invalid_data_type_index
#         with patch.object(csv_service, '_get_column_names', return_value=list(invalid_data_type_index[0].keys())):
#              self.assertFalse(csv_service.validate_csv_data(), "Validation passed with invalid data type (index)")
...
#             }
#         ]
#         csv_service.csv_data = invalid_data_type_price
#         with patch.object(csv_service, '_get_column_names', return_value=list(invalid_data_type_price[0].keys())):
#             self.assertFalse(csv_service.validate_csv_data(), "Validation passed with invalid data type (buy_price)")

#         # Test with empty data (should be valid)
#         csv_service.csv_data = []
#         with patch.object(csv_service, '_get_column_names', return_value=[]):
#             self.assertTrue(csv_service.validate_csv_data(), "Validation failed for empty data")
...
#     #             "last_action": "0"
#     #         }
#     #         csv_service.add_row(new_row)
            
#     #         # Assertions
#     #         self.assertEqual(len(csv_service.csv_data), 3)
#     #         self.assertEqual(csv_service.csv_data[2], new_row)
#     #     finally:
#     #         # Restore original method
#     #         CSVService.__init__ = original_init
...
#     #         rows = csv_service.get_rows_for_sell(115.0)
#     #         self.assertEqual(len(rows), 2)  # Both rows should match
            
#     #         # Test the method with price between thresholds
#     #         rows = csv_service.get_rows_for_sell(107.0)
#     #         self.assertEqual(len(rows), 1)  # Only the first row should match
            
#     #         # Test the method with price below thresholds
#     #         rows = csv_service.get_rows_for_sell(100.0)
#     #         self.assertEqual(len(rows), 0)  # No rows should match
#     #     finally:
#     #         # Restore original method
#     #         CSVService.__init__ = original_init

#     # @patch('main.bots.SCALE_T.csv_utils.csv_service.CSVService._get_epoch_time')
...
#     #     # Monkey patch the class
#     #     original_init = CSVService.__init__
#     #     original_update_order_status = CSVService.update_order_status
...
#     #     # Apply monkey patches
#     #     CSVService.__init__ = mock_init
#     #     CSVService.update_order_status = mock_update_order_status
#     #     CSVService.mock_metadata = self.mock_metadata
...
#     #         with patch.object(csv_service, 'save') as mock_save:
#     #             # Test buy order update
#     #             csv_service.update_order_status(0, 5, 102.0, 'buy')
                
#     #             # Assertions
#     #             self.assertEqual(csv_service.csv_data[0]["held_shares"], 5)
#     #             self.assertEqual(float(csv_service.csv_data[0]["unrealized_profit"]), 5 * (102.0 - 100.0))
#     #             self.assertIsNone(csv_service.csv_data[0]["pending_order_id"])
#     #             self.assertEqual(csv_service.csv_data[0]["last_action"], "1234567890")
#     #             mock_save.assert_called_once()
#     #     finally:
#     #         # Restore original methods
#     #         CSVService.__init__ = original_init
#     #         CSVService.update_order_status = original_update_order_status
...
#     #     # Apply monkey patches
#     #     CSVService.__init__ = mock_init
#     #     CSVService.update_order_status = mock_update_order_
